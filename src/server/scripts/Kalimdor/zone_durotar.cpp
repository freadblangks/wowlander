...
struct npc_durotar_notera : public ScriptedAI
{
    npc_durotar_notera(Creature* creature) : ScriptedAI(creature) { }
        ...
        if (Player* player = unit->ToPlayer())
            if (player->GetDistance(me) < 5.0f)
                if (player->GetQuestStatus(QUEST_YOUNG_AND_VICIOUS) == QUEST_STATUS_INCOMPLETE) {
                    ...
                    player->Dismount();
                    player->UnsummonCreatureByEntry(NPC_SWIFTCLAW);
                }
    }
};
...
struct npc_raptor_38987 : public ScriptedAI
{
    npc_raptor_38987(Creature* c) : ScriptedAI(c) { }

    void OnSpellClick(Unit* clicker, bool& result) override
    {
        Player* player = clicker->ToPlayer();
        if (player->GetQuestStatus(QUEST_BREAKING_THE_LINE) == QUEST_STATUS_INCOMPLETE)
        {
            player->KilledMonsterCredit(38987);
            me->RemoveNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
        }
    }
};
...
void AddSC_durotar()
{
    RegisterCreatureAI(npc_durotar_notera);
    RegisterCreatureAI(npc_raptor_38987);
    new npc_lazy_peon();
    new spell_voodoo();
